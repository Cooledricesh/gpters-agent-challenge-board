...
3. SQL Editor에서 `supabase/schema.sql` 전체를 실행합니다.
   ...
   - 챌린지는 `level`로 기본 과제(`basic`)와 고급 과제(`advanced`)를 구분합니다.
...
3. `/admin`에서 수강생과 챌린지를 추가합니다.
   ...
   - 챌린지: 제목, 기본/고급 구분, 선택 설명을 입력합니다. 날짜는 없고 각 구분 안에서 등록 순서(`order_index`)대로 표시됩니다.
...
2. `/my`에서 기본 과제와 고급 과제로 나뉜 챌린지 완료 여부를 체크/해제합니다.
